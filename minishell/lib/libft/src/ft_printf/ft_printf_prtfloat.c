		...
		unsigned long long *num, unsigned long long *f_num)
{
	...
	if (((utemp + 0.5) * ft_float_pwr(18, 10)) < \
		(ld - *num) * ft_float_pwr(b->flags.pre + 18, 10))
		utemp = (utemp + 1) % (long)ft_float_pwr(b->flags.pre, 10);
	else if (utemp + 0.5 == (ld - *num) * ft_float_pwr(b->flags.pre, 10))
...
void	ft_printf_flopoint(t_block *b, long double ld)
{
	...
	f_num = ((double)((ft_float_pwr(b->flags.pre, 10)) * ft_absld(ld)) \
				- ((ft_float_pwr(b->flags.pre, 10)) * num));
